...
class CustomFrenetGenerator(BaseFrenetGenerator):
    ...
    def crossover(self):
        ...
        if len(candidates) > 4:
            kids_count = 0
            while self.executor.get_remaining_time()["time-budget"] > 0 and kids_count < len(candidates):
                ...
                while self.executor.get_remaining_time()["time-budget"] > 0 and len(kids) > 0:
                    kappas = kids.pop()
                    kids_count += 1
    ...
    def perform_kappa_mutations(self, kappa_mutations, parent, parent_info, extra_info={}):
        ...
        if kappas and len(kappas) > 10:
            i = 0
            while self.executor.get_remaining_time()["time-budget"] > 0 and i < len(kappa_mutations):
